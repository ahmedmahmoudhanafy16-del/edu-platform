...
export function QuizPasscodeModal({
  ...
  async function handleVerify(e?: React.FormEvent) {
    ...
    if (!cleanCode) {
      setErrorMsg(isAr ? 'يرجى إدخال كود الامتحان للمتابعة' : 'Please enter exam access code');
      return;
    }
    ...
    if (cleanCode.startsWith('RETAKE-') || cleanCode.startsWith('RETRY-')) {
      ...
      if (retakeRes.success) {
        ...
        toast.success(retakeRes.message || (isAr ? 'تم تفعيل كود إعادة الامتحان بنجاح!' : 'Retake code redeemed successfully!'));
        onClose();
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-n-800 dark:text-n-700">
                {isAr ? 'كود دخول الامتحان' : 'Exam Access Passcode'}
              </h3>
              <p className="text-xs text-n-400">
                {isAr ? 'امتحان محمي برمز مرور' : 'Protected Exam'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleVerify} className="p-6 space-y-5">
          <div className="space-y-1.5">
            <p className="text-xs font-semibold text-n-700 dark:text-n-600">
              {isAr ? 'الاختبار المطلوب: ' : 'Target Exam: '}
              <span className="font-bold text-accent">{quizTitle}</span>
            </p>
            <p className="text-xs text-n-500 leading-relaxed">
              {isAr
                ? 'يرجى إدخال كود الامتحان للمتابعة (الكود المسلم لك من المعلم):'
                : 'Please enter the access PIN provided by your teacher to begin:'}
            </p>
          </div>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2">
            ...
            >
              {isAr ? 'تأكيد والدخول' : 'Confirm & Enter'}
            </Button>
          </div>
        </form>
      </div>
