...
export default function StudentQuizError({
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center p-4" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="bg-white dark:bg-n-100 border border-n-200 dark:border-n-300 rounded-2xl p-8 max-w-lg w-full text-center shadow-lg space-y-5">
        ...
        <div>
          <h2 className="text-lg font-bold text-n-800 dark:text-n-700">
            {isAr ? 'تعذر تحميل الاختبار حالياً' : 'Unable to load quiz currently'}
          </h2>
          <p className="text-xs text-n-500 dark:text-n-400 mt-1">
            {isAr
              ? 'قد يكون الاختبار غير منشور أو أن هناك تحديثاً جارياً في الخادم.'
              : 'The quiz might not be published or server is undergoing updates.'}
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-3 pt-2">
          ...
          <Link href={`/${locale}/student/quizzes`}>
            <Button variant="secondary" size="md">
              <ClipboardList className={`h-4 w-4 ${isAr ? 'ml-1.5' : 'mr-1.5'}`} />
              {isAr ? 'قائمة الاختبارات' : 'Quizzes List'}
            </Button>
          </Link>
        </div>
      </div>
