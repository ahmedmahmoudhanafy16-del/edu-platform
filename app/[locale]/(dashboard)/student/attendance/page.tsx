    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6" dir={isAr ? 'rtl' : 'ltr'}>
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
          ...
          {isAr ? 'سجل الحضور والغياب في الحصص المباشرة' : 'Live Sessions Attendance Log'}
        </h1>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          {isAr
            ? `مرحباً ${studentName} — توثيق آلي دقيق لنسب الحضور ومدة التواجد في الغرف التفاعلية`
            : `Welcome ${studentName} — Automated accurate logging of attendance and active duration in interactive rooms`}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500">
              {isAr ? 'نسبة الحضور الإجمالية' : 'Overall Attendance Rate'}
            </p>
        ...
        <div className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500">
              {isAr ? 'الحصص المحضورة' : 'Attended Sessions'}
            </p>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800">
          <h2 className="text-sm font-bold text-slate-900 dark:text-white">
            {isAr ? 'سجل الحصص السابقة' : 'Previous Sessions History'}
          </h2>
        </div>
              ...
              <div key={a.id} className="p-4 flex flex-wrap items-center justify-between gap-3 text-xs">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-slate-500 flex items-center gap-1 font-mono">
                    ...
                    {a.durationMin || 45} {isAr ? 'دقيقة' : 'min'}
                  </span>
                  <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 font-bold">
                    {isAr ? 'حاضر ✅' : 'Present ✅'}
                  </Badge>
                </div>
              </div>
