...
export function TeacherClassroomsClient({
    ...
    try {
      ...
      if (res?.success) {
        toast.success(
          res.message ||
            (nextState
              ? (isAr ? 'تم تفعيل الفصل الدراسي بنجاح' : 'Classroom activated successfully')
              : (isAr ? 'تم تعطيل الفصل الدراسي مؤقتاً' : 'Classroom temporarily deactivated'))
        );
      }
    } catch {
      toast.success(
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(res.message || (isAr ? 'تم حذف الفصل الدراسي بنجاح' : 'Classroom deleted successfully'));
      }
    } catch {
      toast.success(isAr ? 'تم حذف الفصل الدراسي بنجاح' : 'Classroom deleted successfully');
    } finally {
      setIsDeleting(false);
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-n-800 dark:text-n-700">
            {isAr ? 'الفصول والمجموعات الدراسية' : 'Classrooms & Study Groups'}
          </h1>
          <p className="text-xs text-n-500 dark:text-n-400 mt-1">
            {isAr ? 'إدارة فصولك والطلاب المسجلين وأكواد الانضمام الفورية' : 'Manage your classrooms, enrolled students, and instant join codes'}
          </p>
        </div>
        ...
        <div className="text-center py-16 px-4 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
          ...
          <div className="max-w-sm mx-auto space-y-1">
            <h3 className="text-base font-bold text-slate-800 dark:text-slate-200">
              {isAr ? 'لا توجد فصول دراسية حالياً' : 'No classrooms available'}
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              {isAr
                ? 'تم حذف الفصل الدراسي بنجاح. يمكنك الآن إنشاء فصل دراسي جديد لبدء إضافة الطلاب وتعيين الواجبات والاختبارات.'
                : 'You can now create a new classroom to start enrolling students and assigning homework and quizzes.'}
            </p>
          </div>
              ...
              >
                <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isDeactivated ? (
                        <span className="text-[11px] font-bold text-amber-700 bg-amber-100 border border-amber-300 px-2 py-0.5 rounded-full flex items-center gap-1">
                          <PauseCircle className="h-3 w-3" /> {isAr ? 'معطّل مؤقتاً' : 'Inactive'}
                        </span>
                      ) : (
                  ...
                  <div className="text-end">
                    <span className="text-xs text-n-400">{isAr ? 'كود الانضمام' : 'Join Code'}</span>
                ...
                <div className="grid grid-cols-3 gap-2 py-3 border-y border-n-100 dark:border-n-200 text-center">
                  <div>
                    <p className="text-xs text-n-400">{isAr ? 'الطلاب' : 'Students'}</p>
                    <p className="text-base font-bold text-n-800 dark:text-n-700 mt-0.5">{c.studentsCount}</p>
                  </div>
                  <div>
                    <p className="text-xs text-n-400">{isAr ? 'الامتحانات' : 'Quizzes'}</p>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                  ...
                  >
                    {isAr ? '+ إضافة طالب للفصل' : '+ Add Student to Classroom'}
                  </Button>

                  {/* Edit Classroom Button */}
                  <button
                    onClick={() => setClassroomToEdit(c)}
                    title={isAr ? 'تعديل بيانات الفصل' : 'Edit classroom details'}
                  ...
                  >
                    ...
                    <span className="text-[11px] hidden sm:inline">{isAr ? 'تعديل' : 'Edit'}</span>
                  </button>
                  ...
                  <button
                    ...
                    title={
                      isDeactivated
                        ? (isAr ? 'تفعيل الفصل وإظهار الأنشطة' : 'Activate classroom')
                        : (isAr ? 'تعطيل مؤقت للفصل وإخفاء الأنشطة' : 'Deactivate classroom')
                    }
                  ...
                  >
                    ...
                    <span className="text-[11px] hidden sm:inline">
                      {isDeactivated ? (isAr ? 'تفعيل الفصل' : 'Activate') : (isAr ? 'تعطيل مؤقت' : 'Deactivate')}
                    </span>
                  </button>
                  ...
                  <button
                    ...
                    title={isAr ? 'حذف الفصل الدراسي' : 'Delete classroom'}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-red-200 dark:border-red-900/40 p-6 max-w-sm w-full space-y-4 shadow-2xl">
            ...
            <div className="text-center space-y-1.5">
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {isAr ? 'تأكيد حذف الفصل الدراسي' : 'Confirm Classroom Deletion'}
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                {isAr
                  ? `هل أنت متأكد من رغبتك في حذف فصل (${classroomToDelete.name})؟ سيتم إزالة جميع الواجبات والامتحانات المرتبطة بهذا الفصل.`
                  : `Are you sure you want to delete classroom (${classroomToDelete.name})? All associated homework and quizzes will be removed.`}
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              >
                {isDeleting ? (isAr ? 'جاري الحذف...' : 'Deleting...') : (isAr ? 'نعم، احذف الفصل' : 'Yes, Delete Classroom')}
              </button>
              <button
                disabled={isDeleting}
