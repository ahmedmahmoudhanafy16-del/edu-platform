  ...
  return (
    <div className="min-h-screen flex flex-col bg-n-50 dark:bg-n-50">
      ...
      <header className="border-b border-n-200 dark:border-n-300 bg-white dark:bg-n-100">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-md bg-accent text-white flex items-center justify-center font-bold text-base">
              🎓
            </span>
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/${locale}/login`}
              className="text-sm font-medium px-4 py-2 rounded-md border border-n-200 dark:border-n-300 text-n-700 dark:text-n-600 hover:bg-n-100 dark:hover:bg-n-200 transition-colors"
            >
              {isAr ? 'دخول المعلم' : 'Teacher Login'}
            </Link>
            <Link
              href={`/${locale}/student-login`}
              className="text-sm font-semibold px-4 py-2 rounded-md bg-accent text-white hover:bg-accent-mid transition-colors"
            >
      ...
      <section className="py-20 px-6 text-center max-w-4xl mx-auto flex-1 flex flex-col justify-center">
        ...
        <p className="mt-4 text-base md:text-lg text-n-500 dark:text-n-500 max-w-2xl mx-auto">
          {isAr
            ? 'بث مباشر عبر المتصفح بدون تحميل برامج، تصحيح ذكي للامتحانات، ونظام متطور لمنع الغش مع بوابة مخصصة لأولياء الأمور.'
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          ...
          >
            {isAr ? 'الدخول بكود الطالب' : 'Enter Student Code'}
          </Link>
          <Link
            href={`/${locale}/login`}
            className="px-6 py-3 rounded-md border border-n-200 dark:border-n-300 bg-white dark:bg-n-100 text-n-800 dark:text-n-700 font-semibold text-base hover:bg-n-100 transition-colors"
          >
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 text-start">
          <div className="p-6 rounded-xl border border-n-200 dark:border-n-300 bg-white dark:bg-n-100">
            ...
            <p className="mt-1 text-sm text-n-500 dark:text-n-400 leading-relaxed">
              {isAr ? 'دخول فوري بكود الحصة بدون الحاجة لتحميل تطبيقات إضافية.' : 'Join instantly via room code with no app download required.'}
            </p>
          </div>
          ...
          <div className="p-6 rounded-xl border border-n-200 dark:border-n-300 bg-white dark:bg-n-100">
            ...
            <p className="mt-1 text-sm text-n-500 dark:text-n-400 leading-relaxed">
              {isAr ? 'رصد مغادرة نافذة الامتحان، ترتيب عشوائي للأسئلة، وتسليم تلقائي عند التكرار.' : 'Tab-switch detection, randomized question pools, and auto-submit.'}
            </p>
          </div>
          ...
          <div className="p-6 rounded-xl border border-n-200 dark:border-n-300 bg-white dark:bg-n-100">
            ...
            <p className="mt-1 text-sm text-n-500 dark:text-n-400 leading-relaxed">
              {isAr ? 'تصحيح سريع للواجبات، علامة مائية لحماية الملفات، وتوليد امتحانات شهرية.' : 'Rapid batch grading, dynamic PDF watermarking, and test generator.'}
            </p>
          </div>
        </div>
      </section>
